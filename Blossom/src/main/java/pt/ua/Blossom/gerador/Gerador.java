...
public class Gerador {
	...
	static int freq = 48; 					// Numero de registos que os monitores fazem por dia.
	static int days = 1;
	static String monitor_type[] = {"Low", "Medium", "High"};     // Gerar monitores especificos
																		   // Low -> ph acido / humidade baixa
																		   // Medium -> ph neutro / humidade média
																		   // High -> ph básico / humidade alta
	...
	public static void main(String[] args) throws InterruptedException{
		...
		for(int i = 0; i<num_of_ph_monitors; i++) {
			...
			MonitorPH ph = new MonitorPH(monitor_type[opt], freq, days);
			monitoresPH.add(ph);
		}
		...
		for(int i = 0; i<num_of_hum_monitors; i++) {
			int opt = getRandomNumberUsingInts(0, 2);
			MonitorHum humid = new MonitorHum(monitor_type[opt], freq, days);
			monitoresHum.add(humid);
		}
		
		if(days == 0) { //Não gerar valores automaticamente, gerar em tempo real
			while(true) {
				
				//Adicionar 1 medição aos monitores de PH
				for(int i = 0; i<monitoresPH.size(); i++) {
					MonitorPH p = monitoresPH.get(i);
					String mode = p.getMode();
					if(mode == "Low") {
						p.generateAcidicSoil(1/freq);
					}
					if(mode == "Medium") {
						p.generateNeutralSoil(1/freq);
					}
					if(mode == "High") {
						p.generateBasicSoil(1/freq);
					}
				}
				
				//Adicionar 1 medição aos monitores de humidade
				for(int i = 0; i<monitoresHum.size(); i++) {
					MonitorHum h = monitoresHum.get(i);
					String mode = h.getMode();
					if(mode == "Low") {
						h.generateDrySoil(1/freq);
					}
					if(mode == "Medium") {
						h.generateNeutralSoil(1/freq);
					}
					if(mode == "High") {
						h.generateMoistSoil(1/freq);
					}
				}
				
				//Esperar tempo real pela proxima medição
				int real_freq = (1/(freq/24));
